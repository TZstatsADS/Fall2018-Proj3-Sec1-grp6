...
setwd("K:/5291/Fall2018-Proj3-Sec1-grp6")
...
Visualize cross-validation results. 
```{r cv_vis}
plot_ly(x=model_values_xgb$max_depth, y=model_values_xgb$eta, z=err_cv_xgb, type="surface")
```
...
if(run.test){
  ...
  tm_test <- system.time(superResolution(LR_dir=test_LR_dir, HR_dir=test_HR_dir_GBM, modelList=fit_train))
}
...
if(run.test){
  ...
  tm_xgb_test <- system.time(superResolution(LR_dir=test_LR_dir, HR_dir=test_HR_dir_XGB, modelList=fit_train_xgb))
}
...
cat("Time for training GBM model=", tm_train[1], "s \n")
cat("Time for GBM super-resolution=", tm_test[1], "s \n")
